...
function printBill(order) {
  ...
  const qrData = JSON.stringify({
    orderId: order.orderId, orderState: order.orderState,
...
<div class="header">
  ...
  <div class="order-side"><div class="order-info"><div class="order-label">ORDER ID</div><div class="order-id">#${order.orderId}</div><div class="order-date">${orderDateFormatted}</div></div><div id="billQrCode" class="qr-box"></div></div>
</div>
<div class="line"></div>
<div class="info-grid">
  <div><div class="section-title">CUSTOMER</div><div class="info-text"><b>${customer?.username||""}</b><br>ID: ${customer?.customerId||""}<br>${customer?.email||""}<br>${customer?.phone||""}<br>Alt: ${customer?.altPhone||""}</div></div>
...
<div class="info-strip">
  ...
  <div><div class="strip-title">PRODUCT DETAILS</div><div class="strip-value">${product?.productName}<br>Size: ${order.size}<br>Qty: ${qty}</div></div>
</div>
<div class="line"></div>
<table class="table">
<tr><th>DESCRIPTION</th><th class="right">QTY</th><th class="right">UNIT PRICE</th><th class="right">AMOUNT</th></tr>
<tr><td>${product?.productName}</td><td class="right">${qty}</td><td class="right">₹${fmt(price)}</td><td class="right">₹${fmt(price)}</td></tr>
<tr><td>Discount (${discount}%)</td><td class="right">-</td><td class="right">-</td><td class="right">- ₹${fmt((price-finalPrice)*qty)}</td></tr>
<tr><td>Subtotal</td><td class="right">${qty}</td><td class="right">₹${fmt(finalPrice)}</td><td class="right">₹${fmt(finalPrice)}</td></tr>
...
<div class="barcode-section"><svg id="billBarcode"></svg><div class="barcode-id">SHIP:${order.orderId}</div></div>
  ...
  const handleShip = useCallback(async (sourceOrder) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        await sendOrderNotification(target.customer?.customerId, "SHIPPED", target.orderId);
      }
    } catch (err) {
  ...
  const handleCancelShip = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        await sendOrderNotification(order.customer?.customerId, "CONFIRMED", order.orderId);
      }
    } catch (err) {
  ...
  const handleCancelConfirm = useCallback(async (reason) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        await sendOrderNotification(order.customer?.customerId, "CANCELLED", order.orderId);
      }
    } catch (err) {
    ...
    <>
      ...
      {lightbox && images.length > 0 && (
        <div onClick={() => setLightbox(false)} style={{
          ...
          {images.length > 1 && (
            <>
              <button onClick={(e)=>{e.stopPropagation();setImgIdx(i=>(i-1+images.length)%images.length);}} style={{position:"fixed",left:18,top:"50%",transform:"translateY(-50%)",width:46,height:46,borderRadius:"50%",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.18)",color:"#fff",fontSize:22,cursor:"pointer",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
              <button onClick={(e)=>{e.stopPropagation();setImgIdx(i=>(i+1)%images.length);}} style={{position:"fixed",right:18,top:"50%",transform:"translateY(-50%)",width:46,height:46,borderRadius:"50%",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.18)",color:"#fff",fontSize:22,cursor:"pointer",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
            </>
          )}
            ...
            <SectionCard title="Customer Details" icon="👤" className="od-section">
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:0}}>
                ...
                <div style={{paddingLeft:20}}>
                  ...
                  {isDone && (
                    <div style={{marginTop:16,padding:"12px 16px",borderRadius:12,background:order.orderState==="CANCELLED"?"rgba(239,68,68,0.07)":"rgba(34,197,94,0.07)",border:`1px solid ${order.orderState==="CANCELLED"?"rgba(239,68,68,0.2)":"rgba(34,197,94,0.2)"}`,color:order.orderState==="CANCELLED"?"#fca5a5":"#86efac",fontSize:12,fontWeight:700,textAlign:"center"}}>
              ...
              <div style={{display:"flex",gap:24,flexWrap:"wrap"}}>
                {images.length > 0 && (
                  <div style={{display:"flex",flexDirection:"column",gap:8,flexShrink:0}}>
                    <div onClick={()=>setLightbox(true)} style={{width:150,height:195,borderRadius:16,overflow:"hidden",background:"rgba(255,255,255,0.06)",cursor:"zoom-in",border:"1px solid rgba(139,92,246,0.3)",boxShadow:"0 8px 32px rgba(0,0,0,0.5)",flexShrink:0,position:"relative"}}>
                      <img src={getImageUrl(images[imgIdx])} alt="product" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} />
                      <div style={{position:"absolute",bottom:8,right:8,background:"rgba(0,0,0,0.55)",borderRadius:8,padding:"3px 7px",fontSize:9,color:"rgba(255,255,255,0.7)",fontWeight:700}}>🔍 Zoom</div>
                    </div>
                    {images.length > 1 && (
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",maxWidth:150}}>
                        {images.map((img,i)=>(
                          <div key={i} className={`od-img-thumb ${i===imgIdx?"active":""}`} onClick={()=>setImgIdx(i)} style={{width:38,height:48}}>
                ...
                <div style={{flex:1,minWidth:220}}>
                  <p style={{color:"#fff",fontWeight:900,fontSize:17,margin:"0 0 2px",lineHeight:1.3}}>{order.product?.productName||"—"}</p>
                  <p style={{color:"rgba(255,255,255,0.3)",fontSize:11,margin:"0 0 14px",fontFamily:"monospace"}}>{order.product?.productId}</p>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16}}>
                    {order.product?.category && <span style={{padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:700,background:"rgba(139,92,246,0.15)",color:"#c4b5fd",border:"1px solid rgba(139,92,246,0.25)"}}>{order.product.category}</span>}
                    {order.product?.subCategory && <span style={{padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:700,background:"rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.5)",border:"1px solid rgba(255,255,255,0.1)"}}>{order.product.subCategory}</span>}
                    {order.product?.status && <span style={{padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:700,background:"rgba(34,197,94,0.12)",color:"#86efac",border:"1px solid rgba(34,197,94,0.25)"}}>{order.product.status}</span>}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:8,marginBottom:16}}>
                    {order.size && <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 12px",border:"1px solid rgba(255,255,255,0.07)"}}><p style={{color:"rgba(255,255,255,0.35)",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",margin:"0 0 3px"}}>Size</p><p style={{color:"#c4b5fd",fontSize:15,fontWeight:900,margin:0}}>{order.size.toUpperCase()}</p></div>}
                    {order.quantity && <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 12px",border:"1px solid rgba(255,255,255,0.07)"}}><p style={{color:"rgba(255,255,255,0.35)",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",margin:"0 0 3px"}}>Quantity</p><p style={{color:"#fbbf24",fontSize:15,fontWeight:900,margin:0}}>{order.quantity}</p></div>}
                    {order.product?.productColor?.name && <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 12px",border:"1px solid rgba(255,255,255,0.07)"}}><p style={{color:"rgba(255,255,255,0.35)",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",margin:"0 0 5px"}}>Colour</p><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:14,height:14,borderRadius:"50%",background:order.product.productColor.hex||"#888",border:"1px solid rgba(255,255,255,0.2)",flexShrink:0}}/><p style={{color:"rgba(255,255,255,0.7)",fontSize:12,fontWeight:700,margin:0}}>{order.product.productColor.name}</p></div></div>}
                  </div>
                  <div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",overflow:"hidden"}}>
                    ...
                    <div style={{padding:"6px 14px"}}>
                      ...
                      {discount > 0 && <InfoRow label={`Discount (${discount}%)`} value={`− ₹${fmt(price-finalPrice)}`} accent="#4ade80" />}
