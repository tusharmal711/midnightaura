...
const ScrollArrow = ({ direction, onClick }) => (
  <button
    ...
    onMouseEnter={e => {
      e.currentTarget.style.background = "rgba(160,120,255,0.22)";
      e.currentTarget.style.borderColor = "rgba(160,120,255,0.7)";
    }}
    onMouseLeave={e => {
      e.currentTarget.style.background = "rgba(14,19,32,0.88)";
      e.currentTarget.style.borderColor = "rgba(160,120,255,0.35)";
    }}
  >
    {direction === "left" ? (
      ...
      try {
        ...
        const results = await Promise.allSettled(
          VALID_CATEGORIES.map((cat) =>
            API.get("/productBuy/fetchProductByCategory", {
              params: { category: cat, page: 1, limit: 10 },
            })
          )
        );
        ...
        results.forEach((res, idx) => {
          ...
          const fetched = (data.data?.products || []).filter((p) => p.id !== product.productId);
          if (fetched.length > 0) grouped[cat] = fetched;
        });
        ...
        setForYouProducts(interleaved);
      } catch (err) {
        console.error("fetchForYou error:", err);
      } finally {
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .foryou-scroll::-webkit-scrollbar{height:0;display:none}
        .foryou-scroll{-ms-overflow-style:none;scrollbar-width:none;}
        .scroll-card-fix, .scroll-card-fix > * { width: 100% !important; height: 100% !important; }
        .scroll-card-fix img { width: 100% !important; object-fit: cover !important; }
      `}</style>
            ...
            <div style={{ marginTop: 56, paddingBottom: 32 }}>
              ...
              {!relatedLoading && relatedProducts.length > 0 && (
                <div style={{ position: "relative", width: "100vw", marginLeft: "calc(50% - 50vw)", marginRight: "calc(50% - 50vw)" }}>
                  <ScrollArrow direction="left" onClick={() => scrollByAmount(relatedScrollRef, "left")} />
                  <ScrollArrow direction="right" onClick={() => scrollByAmount(relatedScrollRef, "right")} />
                  <div
    ...
    <div className="scroll-card-fix" style={{ width: "100%", height: "100%", display: "flex" }}>
      <ProductCard product={p} />
            ...
            <div style={{ marginTop: 56, paddingBottom: 32 }}>
              ...
              {forYouLoading && <RelatedSkeleton />}

              {!forYouLoading && forYouProducts.length === 0 && (
                <div style={{ color: "rgba(255,255,255,0.25)", fontSize: 13, padding: "24px 0" }}>No recommendations found.</div>
              )}
                ...
                window.innerWidth <= 768 ? (
                  ...
                  <div style={{ position: "relative", width: "100vw", marginLeft: "calc(50% - 50vw)", marginRight: "calc(50% - 50vw)" }}>
                    ...
                    <div
                      ...
                      onMouseUp={() => { forYouScrollRef.current.dataset.isDragging = "false"; forYouScrollRef.current.style.cursor = "grab"; }}
                      onMouseLeave={() => { forYouScrollRef.current.dataset.isDragging = "false"; forYouScrollRef.current.style.cursor = "grab"; }}
                    >
                      <div style={{ display: "flex", gap: 14, width: "max-content", paddingBottom: 8 }}>
                        <div style={{ width: 20, flexShrink: 0 }} />
                        {forYouProducts.map((p, i) => (
