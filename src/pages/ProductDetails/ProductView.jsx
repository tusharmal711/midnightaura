...
// ── Encrypt product ID (same as ProductCard) ──────────────────────────────────
const encryptId = (id) =>
...
// ── Resolve image URL from DB/Cloudinary path ─────────────────────────────────
const resolveImageUrl = (path) => {
  if (!path) return null;
  // Cloudinary or any full https URL — return as-is
...
// ── Share Popup ───────────────────────────────────────────────────────────────
// NOTE ON THE FIX:
// Previously, the share text included the raw Cloudinary image URL inline
// (e.g. "🖼️ https://res.cloudinary.com/.../image.jpg"). WhatsApp/Facebook/etc.
// don't turn that into a real rich preview — they just render it as plain,
// ugly link text sitting in the middle of the message (see screenshot #1).
//
// The clean preview card (screenshot #2) only happens when the *page URL*
// itself (the product-view link) has Open Graph meta tags (og:image,
// og:title, og:description) — the receiving app's link-crawler fetches the
// page and builds the card itself. So:
//   1. We no longer inject the raw image URL into any share text.
//   2. Share text is now just product name + price + the page link.
//   3. The actual product-image-in-the-preview-card behavior depends on your
//      backend/SSR setting per-product <meta property="og:image"> tags on
//      the product-view route (see note at bottom of this file).
const SharePopup = ({ visible, onClose, product, shareUrl }) => {
  ...
  // WhatsApp uses the same clean text as everywhere else now — the rich
  // preview (image/title/description) is generated by WhatsApp itself from
  // the page's Open Graph tags when it crawls shareUrl, not from text we send.
  const whatsappEncoded = shareTextEncoded;

  // Facebook: on mobile use the app deep link (fb://), on desktop use sharer
  const facebookHref = mobile
    ? `fb://share?u=${urlEncoded}`
    : `https://www.facebook.com/sharer/sharer.php?u=${urlEncoded}`;

  // Facebook Messenger: on mobile use app, on desktop use web Messenger
  ...
  const shareOptions = [
    {
      ...
      // WhatsApp: wa.me on mobile opens app; web.whatsapp.com on desktop
      href: mobile
        ? `whatsapp://send?text=${whatsappEncoded}`
        : `https://web.whatsapp.com/send?text=${whatsappEncoded}`,
    },
    {
      label: "Facebook",
      ...
      href: facebookHref,
      // On mobile, fallback to sharer if fb:// not installed
      fallbackHref: `https://www.facebook.com/sharer/sharer.php?u=${urlEncoded}`,
      isFbApp: mobile,
    },
    {
      label: mobile ? "Messenger" : "Messenger",
      icon: (
        // Messenger icon (SVG)
        <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2C6.477 2 2 6.145 2 11.243c0 2.826 1.324 5.356 3.408 7.106V22l3.287-1.806A10.8 10.8 0 0 0 12 20.486c5.523 0 10-4.145 10-9.243S17.523 2 12 2zm1.05 12.45-2.55-2.72-4.98 2.72 5.48-5.82 2.62 2.72 4.91-2.72-5.48 5.82z"/>
        </svg>
      ),
      color: "#0084FF",
    ...
    },
    {
      ...
      note: "Copy the link, then paste it in your Instagram story or DM",
    },
  ];
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
              ...
              {mobile && typeof navigator !== "undefined" && navigator.share && (
                <button
                  ...
                  style={{
                    ...
                    color: "#a078ff", fontSize: 12, fontWeight: 600, cursor: "pointer",
          ...
          {/* Product Preview Card (this is the in-app preview only — what WhatsApp
              itself shows on the other end depends on the page's OG tags) */}
          {product && (
          ...
          <div style={{ display: "flex", gap: 10, padding: "0 20px 20px", justifyContent: "center" }}>
            {shareOptions.map((opt) => (
              <a
                key={opt.label}
                href={opt.href}
                target={opt.isFbApp ? "_self" : "_blank"}
                rel="noopener noreferrer"
                title={opt.note || `Share on ${opt.label}`}
                onClick={(e) => {
                  // For FB app links on mobile: try app, fall back to web
                  if (opt.isFbApp && opt.fallbackHref) {
                    e.preventDefault();
                    const appLink = opt.href;
                    const fallback = opt.fallbackHref;
                    // Try to open app; if it fails after 1.5s, open web fallback
                    const fallbackTimer = setTimeout(() => {
                      window.open(fallback, "_blank");
                    }, 1500);
                    // Cancel fallback if app opened (page will blur/hide)
      ...
      >
        {cur?.imageFeedback?.imageData ? (
          <img src={cur.imageFeedback.imageData} alt={`Customer photo ${idx + 1}`} draggable={false}
            style={{ width: "100%", height: "100%", objectFit: mobile ? "cover" : "contain", display: "block" }}
          />
        ) : (
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: mobile ? "70px 18px 36px" : "16px 18px 18px", zIndex: 2 }}>
  ...
  return (
    ...
    >
      ...
      {mobile ? (
        ...
        >
          {resolveImageUrl(images[idx]) ? (
            <img src={resolveImageUrl(images[idx])} alt={`${productName} — ${idx + 1}`} draggable={false}
...
export default function ProductView() {
  ...
  // ── Decrypt URL param → original product ID ──────────────────────────────────
  ...
  // ── Build share URL using the SAME encrypted format as ProductCard navigation ─
  // productId in the URL param is already the encrypted+encoded string from encryptId()
  // So we can use it directly to reconstruct the share URL.
  ...
  return (
    <>
      ...
      {/* Share Popup — share text no longer contains the raw Cloudinary image URL */}
      <SharePopup
        visible={shareOpen}
                        ...
                        {feedbackLoading ? (
                          ...
                          (mobile
                            ...
                            <div key={r._id ?? i} style={{
                              ...
                              minWidth: mobile ? "82vw" : "100%",
                              maxWidth: mobile ? "82vw" : "100%",
                              ...
                              <div style={{
                                ...
                                display: mobile && expandedReview !== i ? "-webkit-box" : "block",
                                WebkitLineClamp: mobile && expandedReview !== i ? 2 : "unset",
                                WebkitBoxOrient: "vertical",
                                overflow: mobile && expandedReview !== i ? "hidden" : "visible",
                              }}>
                ...
                >
                  <div style={{ display: "flex", gap: 14, width: "max-content", paddingBottom: 8 }}>
                    {relatedProducts.map((p, i) => (
                      <div key={p.id ?? p._id ?? i} style={{ width: mobile ? "72vw" : 180, flexShrink: 0, scrollSnapAlign: "start" }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// IMPORTANT — to get the "rich card with the actual product photo" behavior
// shown in your second screenshot, you also need server-side Open Graph tags
// on the /product-view/:productId route, e.g. (Express + a templated index.html,
// or your SSR layer):
//
//   <meta property="og:title" content="{{product.name}} — Midnight Aura" />
//   <meta property="og:description" content="₹{{product.finalPrice}}" />
//   <meta property="og:image" content="{{resolvedCloudinaryUrl}}" />
//   <meta property="og:url" content="{{shareUrl}}" />
//
// This is required because WhatsApp/Facebook/etc. generate the preview card by
// crawling the shared URL server-side — they do NOT read meta tags that React
// injects client-side after the page loads (their crawler doesn't execute JS).
// Without this, no client-side fix can make the receiver's app show your
// product image as a rich preview card.
// ─────────────────────────────────────────────────────────────────────────────